# DocsPI proxy (SpoofDPI 1.2.1) - PowerShell ile derleme ve kopyalama
...
$OutExe = Join-Path $Root "spoofdpi\docspi-proxy.exe"
...
Write-Host "SpoofDPI (docspi-proxy) derleniyor..." -ForegroundColor Cyan
...
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "docspi-proxy.exe") -Force
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "docspi-proxy-x86_64-pc-windows-msvc.exe") -Force
Write-Host "Kopyalandi: src-tauri\binaries\docspi-proxy.exe (ve -x86_64-pc-windows-msvc.exe)" -ForegroundColor Green
