...
async function main() {
    ...
    const exePath = path.join(__dirname, 'src-tauri', 'binaries', 'docspi-proxy-x86_64-pc-windows-msvc.exe');
    const pngPath = path.join(__dirname, 'public', 'docspi-engine.png');
    const iconPath = path.join(__dirname, 'src-tauri', 'icons', 'docspi-engine.ico');
    ...
    if (!fs.existsSync(exePath)) {
        console.log(`Binary not found, skipping icon patch: ${exePath}`);
        return;
    }
    ...
    try {
        await rcedit(exePath, {
            ...
            'version-string': {
                ProductName: 'DocsPI',
                FileDescription: 'DocsPI Service',
                CompanyName: 'aydocs',
